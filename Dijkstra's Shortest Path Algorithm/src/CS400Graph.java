...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if (vertices.get(start) == null || vertices.get(end) == null) {
      throw new NoSuchElementException("Path from start to end vertices search failed.");
    }
    ...
    PQ.add(startPath);
    // makes suree shortest path is found.
    // goes through each path
    while (!PQ.isEmpty()) {
      Path temp = PQ.remove();
      visited.add(temp.start);
      // finds the shortest path by going through all the edges of vertex
      for (int i = 0; i < temp.end.edgesLeaving.size(); i++) {
        if (!visited.contains(temp.end.edgesLeaving.get(i).target)) {
          Path currPath = new Path(temp, temp.end.edgesLeaving.get(i));
          PQ.add(currPath);
        }
      }
      if (temp.end == vertices.get(end)) {
        return temp;// returns the end if current end and actual end match
      }
    }
    // exception for no start or end vertex
    throw new NoSuchElementException("Path from start to end cant be found");
