...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  /*
   * Takes a reference to a newly added red node as its only parameter. Note that this method may
   * also be called recursively, in which case the input parameter may reference a different red
   * node in the tree that potentially has a red parent node. The job of this
   * enforceRBTreePropertiesAfterInsert method is to resolve any red node with red parent property
  ...
  private void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
    if (newNode != root && newNode != null && newNode.parent.blackHeight == 0) {
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    ...
    if (parent.leftChild == child) {
      ...
      // checks if the parent is root node
      if (parent.parent != null) {
        // if not, set the childs parent to parent's parent
    ...
    // an IllegalArgumentException is thrown if the child node is neither right node nor left node
    else {
      throw new IllegalArgumentException(
          "IllegalArgumentException: Provided nodes have incorrect relationship");
    }
  }
  ...
  // test should be designed to test a different case for inserting into a red black tree, and
  // ensuring that your enforceRBTreePropertiesAfterInsert method is correctly
  @Test
  public void test1() {
    ...
    tree.insert(2);
    tree.insert(3);
    assertEquals(tree.toString(), false);
  ...
  public void test2() {
    ...
    tree.insert(4);
    tree.insert(3);
    assertEquals(tree.toString(), "level order: [ 5,5,3,4 ]\n" + "in order: ");
